...
*/
const calculateTransforms = (el, to, from, parentTo, parentFrom) => {
    ...
    return {
        apply() {
            if (el.classList.contains("circle"))
                console.log(el, to.styles, from.styles);
...
function motion2(el, trigger) {
    ...
    function elementsFromTree(branch) {
        const els = [];
        const pushBranchEl = (branch) => {
            els.push(branch.el);
    ...
    function invert(branch, parentBranch) {
    ...
    function makeBranches(el) {
        return {
            ...
            children: Array.from(el.children).map(makeBranches)
        };
    }
}
const motion = (el, className) => {
...
const card = document.querySelector(".befaux .card");
const motionBtns = Array.from(document.querySelectorAll("[data-motion]"));
const testEls = Array.from(document.querySelectorAll(".befaux > div"));
